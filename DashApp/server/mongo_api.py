...
class KeyCharacteristics(Resource):
    def get(self, company):
        return db_key_characteristics.find_one({"company": company})["key_characteristics"]

class StockPrice(Resource):
    def get(self, symbol):
        return db_stock_price_lasthour.find_one({"company": symbol})["stock_price_onehour"]
...
class InternationalDaxNews(Resource):
    def get(self, company):
        cursor = db_international_dax_news.find(
            {"id": company}
        )
