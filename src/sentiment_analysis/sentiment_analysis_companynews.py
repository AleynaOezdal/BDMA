...
df2 = headlinesdf.select(col("company"),
    ...
    col("news.time"),
    col("time"))
df2Flatten = df2.toDF("company","more_info","headline","time","timestamp")
...
df_resultsFlatten = df_results.toDF("company", "more_info", "headline","time", "timestamp", "class")
...
finished_df.writeStream \
    ...
    .option("spark.mongodb.collection", "company_news_gcp") \
