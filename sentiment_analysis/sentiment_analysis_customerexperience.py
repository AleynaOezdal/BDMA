...
df = (
    ...
    .option("collection", "Customer_experience")
    .load()
)
...
document = DocumentAssembler().setInputCol("review").setOutputCol("document")
...
df_resultsFlatten = df_results.toDF(
    "id", "more_info", "review", "title", "time", "timestamp", "class"
)
...
).option("collection", "Customer_experience_sentiment").save()
