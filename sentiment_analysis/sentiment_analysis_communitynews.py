...
df = spark.read.format("mongodb").option("database", "Company-Experience").option("collection", "Community_news").load()
...
df_resultsFlatten = df_results.toDF("id", "more_info", "user", "message", "time", "timestamp", "class")
...
finished_df.write.format("mongodb").mode("append").option("database", "Company-Experience").option("collection", "Community_news_sentiment").save()
